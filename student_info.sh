...
echo "$($PSQL "SELECT course FROM courses LEFT JOIN majors_courses USING(course_id) LEFT JOIN majors USING(major_id) LEFT JOIN students USING(major_id) GROUP BY course HAVING COUNT(DISTINCT(student_id)) = 1 ORDER BY course")"
